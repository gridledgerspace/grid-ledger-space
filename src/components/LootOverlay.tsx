...
export default function LootOverlay() {
  ...
  // 🔥 ВИПРАВЛЕННЯ: Додано "as number" в кінці
  const currentLoad = Object.values(cargo || {}).reduce((a: any, b: any) => a + b, 0) as number
  
  // Тепер TS знає, що currentLoad - це число, і помилка зникне
    ...
    <div className="fixed inset-0 z-40 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in">
      <div className="w-full max-w-md bg-space-950 border border-neon-cyan/50 rounded-xl shadow-[0_0_50px_rgba(0,240,255,0.1)] overflow-hidden flex flex-col">
        ...
        <div className="p-4 bg-black/40 border-b border-white/10 flex justify-between items-center">
            ...
            <button onClick={closeLoot} className="text-gray-500 hover:text-white transition-colors">
                <X size={24} />
            </button>
        </div>
        ...
        <div className="bg-black/20 px-4 py-2 flex justify-between items-center text-xs font-mono border-b border-white/5">
            ...
            <span className={isFull ? 'text-red-500' : 'text-neon-cyan'}>
                {currentLoad} / {maxCargo}
            </span>
        </div>
                    ...
                    <div key={idx} className="flex items-center justify-between p-3 bg-white/5 rounded border border-white/5 hover:border-neon-cyan/30 transition-all group">
                        <div className="flex items-center gap-3">
                            <div className={`w-8 h-8 flex items-center justify-center rounded ${
                                item.type === 'weapon' ? 'bg-red-500/20 text-red-500' :
                                item.type === 'module' ? 'bg-purple-500/20 text-purple-500' :
                            ...
                            <div>
                                ...
                                {!item.amount && <div className="text-[10px] text-purple-400 uppercase">{item.type}</div>}
                            </div>
                        </div>
                        
                        <button 
                            onClick={() => takeLootItem(idx)}
                            className="p-2 bg-black/40 rounded border border-white/10 text-neon-cyan hover:bg-neon-cyan hover:text-black transition-all"
                            title="Take Item"
                        >
                            <Download size={16} />
                        </button>
        ...
        <div className="p-4 bg-black/40 border-t border-white/10 flex gap-3">
            <button 
                onClick={takeAllLoot}
                disabled={lootContainer.length === 0}
                className="flex-1 bg-neon-cyan text-black font-bold py-3 rounded text-sm uppercase tracking-wider hover:bg-white transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
            >

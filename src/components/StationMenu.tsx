...
import { X, Wrench, Activity, ShieldCheck, ShoppingBag } from 'lucide-react'
  ...
  return (
    <div className="absolute inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center animate-in fade-in p-4">
      <div className="glass-panel w-full max-w-4xl border border-neon-cyan/50 shadow-[0_0_50px_rgba(0,240,255,0.1)] flex flex-col max-h-[90vh]">
        ...
        {/* === HEADER (Оновлений) === */}
        <div className="flex justify-between items-center p-4 md:p-6 border-b border-white/10 bg-black/40 relative">
            
            {/* Ліва частина: Назва */}
            <div className="flex items-center gap-4">
                <div>
                    <h2 className="text-xl md:text-2xl font-black text-neon-cyan flex items-center gap-2 md:gap-3 uppercase tracking-widest">
                        <ShoppingBag className="hidden md:block" size={24} /> STATION MARKET
                    </h2>
                    <p className="text-[10px] md:text-xs text-gray-400 font-mono mt-0.5">SECTOR 0:0 TRADE HUB</p>
                </div>
            </div>

            {/* Права частина: Баланс + Кнопка закриття */}
            <div className="flex items-center gap-4 md:gap-8">
                 {/* 🔥 БАЛАНС ПЕРЕНЕСЕНО СЮДИ */}
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-6 grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
            ...
            <div>
                {/* Заголовок секції (без балансу, бо він тепер зверху) */}
